...
pub const Surface = extern struct {
    ...
    pub fn grabFocus(self: *Self) void {
        // Focus the GhosttySurface itself (not the inner render surface)
        // so the focused object exposes role=terminal and GtkAccessibleText
        // to AT-SPI. The EventControllerKey attached to the template root
    ...
    fn axDeviceScale(self: *Self) f32 {
        const scale = self.private().render_surface.as(gtk.Widget).getScaleFactor();
    ...
    fn renderSurfacePresent(
        _: *RenderSurface,
        self: *Self,
    ) callconv(.c) void {
        // A new frame reached the screen. If an AT client is actively
        // querying, check whether the terminal text actually changed since
        // the last notification. We only emit the AT-SPI change events when
        // something actually changed, so we don't interrupt Orca (or any
        // other AT client) mid-read on every rendered frame.
        //
    ...
    pub const Class = extern struct {
        ...
        fn init(class: *Class) callconv(.c) void {
            ...
            class.bindTemplateCallback("render_surface_present", &renderSurfacePresent);
